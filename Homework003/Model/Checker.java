...
public class Checker implements IChecker {
    ...
    public boolean checkData(String[] data) throws Exception{
        for(int i = 0; i < 3; i++)
            if (!checkName(data[i])) throw new Exception("Недопустимое значение ФИО");
        checkDate(data[3]);
    ...
    private boolean checkDate(String date) throws Exception {
        ...
        if (checkCountOfData(arrayDate, 3) != 0) throw new Exception("Неверный формат даты рождения");
        if (!checkStringAsNumber(arrayDate[0], 1, 31))
            throw new Exception("Недопустимое значение дня рождения");
        if (!checkStringAsNumber(arrayDate[1], 1, 12))
            throw new Exception("Недопустимое значение месяца рождения");
        if (!checkStringAsNumber(arrayDate[2], 1900, 2023))
    ...
    private boolean checkPhoneNumber(String phoneNamber) throws Exception {
        if (!phoneNamber.chars().allMatch(Character::isDigit))
            throw new Exception("Неверный формат номера телефона");
        String[] arrayDate = parser.parseOfData(phoneNamber, "");
    ...
    private boolean checkStringAsNumber(String value, int min, int max) {
